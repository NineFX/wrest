// Command mtls-server is a minimal TLS server requiring a client
// certificate, used by wrest's CI to prove a tls::Identity from the
// Windows store is actually presented during a handshake.
//
...
func main() {
	...
	mux.HandleFunc("/client-cert", func(w http.ResponseWriter, r *http.Request) {
		if r.TLS == nil || len(r.TLS.PeerCertificates) == 0 {
			// Unreachable with RequireAnyClientCert, but a 403 is a far
			// clearer CI failure than a panic.
...
// selfSignedServerCert builds a throwaway server certificate for
// 127.0.0.1.  The test disables server certificate validation -- it is
// exercising client authentication, not chain building -- so this only
